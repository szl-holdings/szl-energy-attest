...
> **📦 Canonical energy package (Wave D consolidation).** This repo is the **canonical SZL energy package**. The former `szl-holdings/governed-inference-meter` has been **folded in** here: its unique *live inference metering* code — the NVML `EnergyMeter` (hardware energy counter **+ power-integral / trapezoidal fallback**), an advisory policy gate, and the `meter()` / `metered()` inference wrappers that emit tokens-per-joule hash-chained receipts — now lives under [`szl_energy_attest.inference_meter`](./szl_energy_attest/inference_meter). `governed-inference-meter` is now **DEPRECATED** and points here (it was **not deleted**; archiving is a later founder step). The duplicated receipt/attestation/PCGI-spine layers were **not** re-copied — this package already provided them. Λ = **Conjecture 1** (advisory) is preserved verbatim; no conjecture is upgraded to proven.
